...
from assignment_manager.models import Assignment, Course, Group as db_Group
...
class CommonSetupTestCase(TestCase):
    def setUp(self):
        ...
        self.group1 = db_Group.objects.create(name="TestGroup1", course=self.course, creation_date=timezone.now())
        self.group1.assignments.add(self.asg)
        self.group2 = db_Group.objects.create(name="TestGroup2", course=self.course, creation_date=timezone.now())
...
class TestCourseSubgroup(CommonSetupTestCase):
    ...
    @patch('gitlab.v4.objects.GroupManager.get')
    @patch('gitlab.v4.objects.GroupManager.create')
    def test_creation_success(self, mock_create, mock_get):
    ...
    def test_creation_failure(self, mock_create, mock_get):
        ...
        def mock_get_side_effect(arg):
            if arg == f"mude-project-x/{self.asg.course.unique_name}":
        ...
        mock_get.assert_called_with(f"mude-project-x/{self.asg.course.unique_name}")
    ...
    # todo: see if this test should exist. right now it tests the wrong functionality;
    #   a call to the gitlabmanager should not influence the db
